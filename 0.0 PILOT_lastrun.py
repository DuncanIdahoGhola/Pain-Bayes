...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\labmp\\Desktop\\git\\replay_pain\\Pain-Bayes\\0.0 PILOT_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('exit_break') is None:
        # initialise exit_break
        exit_break = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='exit_break',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    break_time = visual.TextStim(win=win, name='break_time',
        ...
        languageStyle='LTR',
        depth=-1.0);
    exit_break = keyboard.Keyboard(deviceName='exit_break')
    ...
    # Run 'Begin Routine' code from save_param
    PAIN_THRESHOLD = float(expInfo['threshold'])
    PAIN_TOLERANCE = float(expInfo['tolerance'])
    
    # Computed intensity levels (I_base per condition)
    painful_range = PAIN_TOLERANCE - PAIN_THRESHOLD
    expInfo['pain_intensity_low_mA']  = str(round(PAIN_THRESHOLD + PAIN_PROPORTIONS['low']  * painful_range, 4))
    expInfo['pain_intensity_high_mA'] = str(round(PAIN_THRESHOLD + PAIN_PROPORTIONS['high'] * painful_range, 4))
    ...
    conditions_loop = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('condition_files_pilot/001_conditions_pilot.csv'), 
        seed=None, 
    )
    ...
    for thisConditions_loop in conditions_loop:
        ...
        break_2 = data.Routine(
            name='break_2',
            components=[break_time, exit_break],
        )
        break_2.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from break_code
        elapsed = experiment_clock.getTime()
        
        if elapsed - last_break_time >= break_interval:
            continueRoutine = True  # show the break screen
            last_break_time = experiment_clock.getTime()
        else:
            continueRoutine = False  # skip the break screen, go straight to trial
        # create starting attributes for exit_break
        exit_break.keys = []
        exit_break.rt = []
        _exit_break_allKeys = []
        ...
        while continueRoutine:
            ...
            # if break_time is starting this frame...
            if break_time.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'break_time.started')
                # update status
                break_time.status = STARTED
                break_time.setAutoDraw(True)
            
            # if break_time is active this frame...
            ...
            # if exit_break is starting this frame...
            if exit_break.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                exit_break.frameNStart = frameN  # exact frame index
                exit_break.tStart = t  # local t and not account for scr refresh
                exit_break.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(exit_break, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'exit_break.started')
                # update status
                exit_break.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(exit_break.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(exit_break.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if exit_break.status == STARTED and not waitOnFlip:
                theseKeys = exit_break.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
                _exit_break_allKeys.extend(theseKeys)
                if len(_exit_break_allKeys):
                    exit_break.keys = _exit_break_allKeys[-1].name  # just the last key pressed
                    exit_break.rt = _exit_break_allKeys[-1].rt
                    exit_break.duration = _exit_break_allKeys[-1].duration
                    # a response ends the routine
                    continueRoutine = False
        ...
        # check responses
        if exit_break.keys in ['', [], None]:  # No response was made
            exit_break.keys = None
        conditions_loop.addData('exit_break.keys',exit_break.keys)
        if exit_break.keys != None:  # we had a response
            conditions_loop.addData('exit_break.rt', exit_break.rt)
            conditions_loop.addData('exit_break.duration', exit_break.duration)
